...
function callTakeHostage()
	...
	if not canTakeHostage then 
		...
		TriggerEvent('wasabi_notify:notify', 'Take hostage', 'You need a pistol with ammo to take a hostage at gunpoint!', 7000, 'error', true, icon, 'TakeHostage')
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerEvent('wasabi_notify:notify', 'Take hostage', 'There are no people to take hostage!', 7000, 'error', true, icon, 'TakeHostage')
				-- drawNativeNotification("~r~There are no people to take hostage!")
			end
		else
			TriggerEvent('wasabi_notify:notify', 'Take hostage', 'There are no people to take hostage!', 7000, 'error', true, icon, 'TakeHostage')
			-- drawNativeNotification("~r~There are no people to take hostage!")
